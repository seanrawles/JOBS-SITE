...
$showdate = date("Y-m-d",strtotime($show_date));
	$sql = "UPDATE `jobs_canjobs` SET `job_name`='$job_name_clean',`comments`='$comments_clean',`attachment2`='$attachment2',`due_date`='$newdate',`show_date`='$showdate',`status`='$status',`userid`='$userid',`AE`='$AE',`Property`='$property' WHERE job_numb = $job_numb";
$conn->exec($sql);
    }
...
$to = "mnarayan@canneryresorts.com, jmathews@canneryresorts.com, $user_email";
...
header('Location: http://jobs.canneryresorts.com/job_list-admin2.php?orderBy=job_numb');
?>